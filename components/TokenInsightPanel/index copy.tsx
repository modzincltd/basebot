...
export default function TokenInsightPanel({ tokenAddress }: { tokenAddress: string }) {
  ...
  const generateSignal = (stats: any) => {
    ...
    if (priceChange > 5 && numNetBuyers > 100) return "🔥 Strong Buy Signal"
    if (priceChange > 3 && buyVolume > sellVolume) return "📈 Buy Momentum"
    if (priceChange < -5 && numNetBuyers < 0) return "🔻 Strong Sell Signal"
    if (priceChange < -3 && sellVolume > buyVolume) return "⚠️ Sell Pressure"
  ...
  return (
    <div className="bg-white shadow rounded p-6 mx-auto space-y-6">
      ...
      {[{ label: "5 min", stats: stats5m }, { label: "1 hour", stats: stats1h }, { label: "6 hour", stats: stats6h }, { label: "24 hour", stats: stats24h }].map(({ label, stats }) => (
        <div key={label} className="border-t pt-4">
          <h3 className="font-semibold text-lg mb-1">📈 Stats ({label})</h3>
          <p className="mb-2">{generateSignal(stats)}</p>
